    ...
    <Paper component={motion.div} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}
      ...
      <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap', mb: 1 }}>
        {!loading && pred.length && years.length && pred.map((pv, idx) => (
          <Paper elevation={1} key={idx} sx={{ px: 2.2, py: 1.1, borderRadius: 3, background: theme.palette.mode==='dark' ? '#223c4e' : '#f7faff', minWidth: 70 }}>
            ...
            {mode === 'whatif' && deltaAt(idx) !== null && (
              <Box sx={{ display: 'flex', justifyContent: 'center', mt: 0.4 }}>
                <Chip size="small" label={`${deltaAt(idx) >= 0 ? '+' : ''}${deltaAt(idx)}`} sx={{
                  height: 20,
                  fontSize: 12,
                  color: deltaAt(idx) >= 0 ? theme.palette.success.main : theme.palette.error.main,
                  background: subtle,
                  borderRadius: 1.5,
                }} />
              </Box>
